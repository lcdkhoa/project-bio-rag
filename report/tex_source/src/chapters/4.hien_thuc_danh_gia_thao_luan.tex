...
Chương này trình bày toàn diện quá trình thực nghiệm, kiểm thử định lượng và thảo luận khoa học về hiệu năng của hệ thống trợ lý ảo. Nội dung bao gồm: thiết lập môi trường và công cụ thực nghiệm; quy mô và cấu trúc kho tri thức số hoá; phương pháp xây dựng bộ dữ liệu kiểm thử có gắn nhãn nguồn xác thực; hệ thống các độ đo đánh giá độc lập cho hai pha: pha truy xuất tri thức và pha sinh ngôn ngữ; các kết quả thực nghiệm chi tiết trên \textbf{240 câu hỏi kiểm thử} phủ rộng 12 cuốn sách giáo khoa; và cuối cùng là phần thảo luận chuyên sâu về các phát hiện khoa học, ưu điểm cùng các giới hạn kỹ thuật được nhận diện qua thực nghiệm.
...
Mô hình giám khảo & Tập hợp $4$ mô hình LLM xoay vòng qua Groq API (Mục~\ref{sec:judge_pha2}) \\
\bottomrule
\end{tabular}
\end{table}
...
Nhằm đảm bảo tính khách quan khoa học, mô hình giám khảo chấm điểm được phân tách độc lập hoàn toàn với mô hình sinh ngôn ngữ Qwen2.5, tuân thủ nghiêm ngặt phương pháp mô hình ngôn ngữ làm giám khảo độc lập LLM-as-a-judge~\cite{zheng2023llmjudge} nhằm loại bỏ thiên kiến tự chấm điểm thiên vị của mô hình.
...
\begin{itemize}
    ...
    \item \textbf{Nhóm câu hỏi ngoài phạm vi:} Nhóm này tập hợp các câu hỏi thuộc các môn học khác (Lịch sử, Địa lí, Ngữ văn, Tin học, Toán học...). Về mặt thiết kế, nhóm này \textbf{không có tài liệu nguồn sách giáo khoa KHTN tương ứng}, và phản hồi chuẩn của hệ thống là \textbf{phải nhận biết câu hỏi ngoại vi và từ chối trả lời}, đóng vai trò kiểm chứng định lượng cho khả năng phòng chống ảo giác tri thức (Mục~\ref{sec:ablation}).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Pha 2 --- Đánh giá chất lượng sinh ngôn ngữ:} Ứng dụng phương pháp mô hình ngôn ngữ làm giám khảo độc lập LLM-as-a-judge~\cite{zheng2023llmjudge, liu2023geval, es2024ragas, min2023factscore, saadfalcon2023ares} với mô hình ngôn ngữ độc lập chấm điểm định lượng theo ba tiêu chí cốt lõi.
\end{enumerate}
...
\midrule
BM25 thuần (Vector thưa) & 0,6636 & 0,0905 & 0,1475 & 0,1796 & 0,2303 \\
Ngữ nghĩa thuần (Vector đặc) & 0,5664 & 0,0647 & 0,1338 & 0,1778 & 0,2302 \\
...
Bảng~\ref{tab:ablation_rerank} phân tích chi tiết đóng góp độc lập của bước tái xếp hạng bằng mô hình tương tác chéo trên từng kênh truy xuất riêng biệt (ở quy mô thử nghiệm $50$ ứng viên ban đầu, tắt cổng lọc).

\begin{table}[H]
\centering
\caption{Đóng góp của bước tái xếp hạng bằng Cross-Encoder trên các kênh truy xuất ($210$ câu hỏi) (Nguồn: Kết quả thực nghiệm của đồ án)}
...
\begin{enumerate}
    ...
    \item \textbf{Kênh ngữ nghĩa vector đặc chịu sự suy thoái lớn trước ký hiệu khoa học và cần sự hỗ trợ của mô hình tái xếp hạng:} Đối với miền dữ liệu chứa mật độ cao công thức và thuật ngữ chuyên ngành, kiến trúc hai nhánh mã hóa độc lập Bi-Encoder (kích thước $1024$ chiều) gặp khó khăn trong việc phân biệt các ký tự định danh chính xác, dẫn tới chỉ số R@1 ban đầu thấp ($0{,}0647$). Bước tái xếp hạng đã phát huy tối đa hiệu quả khi nâng chỉ số này lên $0{,}0994$ (tăng $+53{,}6\%$).
