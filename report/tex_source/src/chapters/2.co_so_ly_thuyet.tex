...
\begin{itemize}
    ...
    \item \textbf{Pha Truy vấn và Tạo sinh:} Khi người dùng gửi câu hỏi, hệ thống tiến hành nhúng câu hỏi thành vector truy vấn tương ứng. Cơ chế tìm kiếm độ tương đồng trong cơ sở dữ liệu vector sẽ xác định các đoạn văn bản có độ tương đồng ngữ nghĩa cao nhất để trích xuất thành tập ngữ cảnh liên quan. Phương pháp này --- còn được gọi là truy xuất ngữ nghĩa theo vector đặc~\cite{karpukhin2020dpr} --- cho phép tìm kiếm chính xác nội dung đồng nghĩa ngay cả khi không có sự trùng khớp hoàn toàn về mặt từ khóa, khắc phục nhược điểm của các thuật toán so khớp từ khóa thuần túy như BM25~\cite{robertson2009bm25}. Cuối cùng, lời nhắc có cấu trúc kết hợp giữa ngữ cảnh tài liệu và câu hỏi gốc sẽ được đưa vào mô hình ngôn ngữ để tạo lập câu trả lời hoàn chỉnh có căn cứ học liệu rõ ràng.
\end{itemize}
