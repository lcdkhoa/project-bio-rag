...
|---|---|---|
| MT1 | Truy vấn đa định dạng: văn bản + **công thức** + hình/sơ đồ/biểu đồ | **XONG — ETL THẬT ĐÃ CHẠY, ĐO XÁC NHẬN** (D-162). Lượt Colab 8: 4293/16515 chunk merge MinerU thành công (`applied`+`unmatched_count`), 0 lỗi gọi model, đủ 12/12 quyển, BM25 dựng lại khớp. Bộ đánh giá viết lại theo D-182 (lấy mẫu ngẫu nhiên, không theo quyển) đã chạy `--n 240` thật + đưa vào `report/tex_source/` (D-187, D-190, 2026-09-05) — còn nợ duy nhất: bake-off CER/tỉ lệ lỗi dấu MỚI đo lại trên văn bản đã merge để lượng hoá mức cải thiện chỉ số dưới |
...
| Test suite | **XANH** — `pytest tests/ -q` → **732 passed, 3 skipped** (đo lại 2026-09-06, 48,3 s) | Con số **709 pass / 13 skip** ghi ở đây từ D-182 (2026-09-04) **đã cũ**: D-190 bỏ `skipif` của `tests/test_bao_cao_so_lieu.py` (CSV eval nay đã commit nên luôn tồn tại) nên 2 test từng SKIP giờ chạy thật, cộng test mới của D-184..D-190. **CẢNH BÁO còn nguyên giá trị:** "xanh" chỉ nghĩa là các cổng ĐANG BẬT còn đúng — một cổng bị vô hiệu bằng SKIP vẫn cho suite xanh (đúng lớp lỗi D-182 đã dính). Các con số **769**, **761**, **686**, **488**, **584**, **617**, **781**, **709** ghi ở đây trước đó đều đã cũ |
...
| Bộ test câu hỏi (D-182: lấy mẫu ngẫu nhiên) | **XONG, ĐÃ CHẠY THẬT `--n 240` + DUYỆT TAY (D-187, 2026-09-05)** | Cấu trúc cố-định-theo-quyển (192 văn bản 16/quyển + 48 hình 4/quyển) mô tả ở các mục trên đã bị **HUỶ HOÀN TOÀN** (D-182). Bộ hiện hành: `src/test/eval_results/draft.csv` (đã commit), **240 câu = 158 văn bản + 52 hình + 30 ngoài phạm vi**, lấy mẫu ngẫu nhiên toàn corpus (`p_hinh=0,2483` tính từ `n_chunk≈11444/n_anh≈3780`), phân bố theo NXB **không đều** (CD 68 · CTST 72 · KNTT 70 · ngoài-phạm-vi 30). Duyệt bằng một lượt đọc toàn file (`meta.json human_reviewed:true`, `tao_luc`→`reviewed_at` cách 3g14p43s ≈ 48,7 giây/câu) — KHÔNG còn phiếu HTML riêng từng câu hình như quy trình cũ, nên không có thống kê "%câu bị sửa" tách riêng cho bộ này |
| Đánh giá đầu-cuối bộ câu (D-182: theo LOẠI, không theo quyển) | **XONG, ĐÃ CHẠY THẬT TRÊN GPU (D-187, 2026-09-05), ĐÃ ĐƯA VÀO `report/tex_source/` (D-190, 2026-09-05)** | Nguồn: `src/test/eval_results/` (đã commit). Theo LOẠI câu hỏi: văn_bản (158 câu) Correct 4,19 · Faithful 4,56 · Relevancy 4,59; hình (52 câu) Correct 3,08 · Faithful 3,65 · Relevancy 3,90; ngoài_phạm_vi (30 câu) Correct 4,87 · Faithful/Relevancy 5,00, tỉ lệ từ chối đúng 29/30=0,9667. Bảng 4 phương pháp @ n=50 (`retrieval_report.csv`): keyword MRR 0,6636 · dense 0,5664 · truyền_thống (hybrid rerank off) 0,6947 · **đề_xuất (hybrid rerank on) 0,7789** — thắng mọi cột TRỪ R@10 (0,2458 vs 0,2643 của truyền_thống, giả thuyết `RERANK_SCORE_MIN=0,59` lọc bớt ứng viên đúng, CHƯA xác nhận, câu hỏi mở). **Đơn vị đo đã đổi** so với mọi lượt trước: `R@k` nay là tỉ lệ chuẩn `|top-k∩gold|/m` (D-182), không phải hit@k nhị phân — nên KHÔNG so trực tiếp được với "Recall(page) 0,8833" của các lượt D-173/D-175 trở về trước; khái niệm "trần recall thô 0,9042" của D-132 không còn tồn tại ở dạng cũ (công cụ mới không tính per-book/page-level hit@k nữa). Trục tổng hợp nay là LOẠI câu hỏi (chỉ đạo CBHD, D-181 #5), **không còn bảng xếp hạng 12 cuốn sách** như các lượt trước — một đánh đổi có chủ đích, ghi thành hạn chế trong báo cáo (D-190). M2C (MT4 vế ii, D-189): Correct Δ−0,077 · Faithful/Relevancy Δ−0,038 mỗi tiêu chí, 46/52 câu không đổi — **kết luận dứt khoát: không cải thiện**, giữ `MULTIMODAL_CONTEXT_ENABLED=false`. **Số 231/238 câu, "Recall(page)", "trần recall thô" ở các dòng TRƯỚC trong bảng này đều là LỊCH SỬ** — đo trên cấu trúc bộ test/công cụ cũ đã bị D-182 thay thế, không dùng làm chuẩn nghiệm thu nữa |
...
hình đúng chủ đề. **`--n 240` thật + duyệt tay: ĐÃ CHẠY (D-187, 2026-09-05)**
và đã đưa vào `report/tex_source/` (D-190, 2026-09-05) — hai việc từng ghi
"CHƯA CHẠY"/"chưa cập nhật" ở đoạn này khi viết D-182 nay đều xong.
...
D-182 đổi cấu trúc đo lường.** Cập nhật `report/tex_source/` với bộ số này
được để một phiên làm việc RIÊNG lúc viết dòng này — **đã xong ở D-190,
2026-09-05**, xem dòng "Báo cáo" trong bảng ở trên. `src/test/eval_results/`
...
**Đã đưa vào `report/tex_source/`** (D-190, 2026-09-05, mục `sec:mm` Chương 4).
