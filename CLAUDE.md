...
- **Decisions:** `document/decision_log.html` (data-driven `DECISIONS[]` log; every decision is recorded here — currently **D-01…D-108**).
...
| Test suite | **XANH** | `pytest tests/ -q` → **617 pass, 3 skip, 24,1 s** (2026-08-25 sau M3 + bộ 240; các con số **488** và **584** ghi ở đây trước đó đã cũ) |
...
| Kho ảnh **12/12 quyển** | **XONG — 3 880 doc** (D-110, D-111, D-121, D-124) | Lượt 1: 15:33→20:35 = **5 h 01**, 12/12 quyển, 3 143 doc. Rồi phát hiện **lỗi ▲ của CTST** (xem dòng dưới) nên dựng lại 4 quyển CTST 20:46→23:17. ▸ **Độ phủ nhãn hình, đo bằng `python -m src.test.qa_figure_coverage`:** CD **92/92/96/97%** · KNTT **95/95/96/95%** · CTST **72/83/88/89%**. Chỉ `6_CTST` (72%) dưới ngưỡng 0,80. ▸ doc/quyển: `6_CD` 418 · `6_CTST` **470** · `6_KNTT` 285 · `7_CD` 269 · `7_CTST` **335** · `7_KNTT` 203 · `8_CD` 324 · `8_CTST` **423** · `8_KNTT` 215 · `9_CD` 379 · `9_CTST` **324** · `9_KNTT` 235 |
...
| **Báo cáo (`report/tex_source/`)** | **Ch.1--3 XONG; Ch.4 xong §4.1--4.5.1; §4.5.2--4.6 + Ch.5 + Tóm tắt CÒN NGUYÊN VĂN BÁO CÁO CŨ** (D-117, D-118, D-119, **D-129**) | Source `.tex` người dùng cung cấp 2026-08-25. ▸ Bìa: tên đề tài lấy **nguyên văn `goal.docx` đã ký** (đọc bằng `python-docx`), loại → **ĐỒ ÁN TỐT NGHIỆP**. ▸ Ch.1 theo MT1–MT5 của đề cương + luận điểm *khoảng cách recall*; Ch.2 thêm mục BM25/RRF/cross-encoder (trước đó BM25 xuất hiện **1 lần** và như phương pháp ĐỐI LẬP); Ch.3 sửa 9 chỗ + viết lại §3.3.2; Ch.4 mục 4.1/4.3/4.4. ▸ **Bảng môi trường cũ ghi SAI**: *Colab Pro + A100 40GB*, thật là máy trạm **CPU** (`torch 2.11.0+cpu`, 16 lõi, 63,9 GB). ▸ **Bỏ dung sai ±1 trang** ở công thức `rel(d)` — lập luận cũ không đúng với pipeline hiện tại. ▸ Máy **không có LaTeX** → `report/kiem_tra_tex.py` kiểm ref/cite/gói/ký tự điều khiển/**số cũ**; nay **thoát 1 với 9 số cũ còn sót**. ▸ **D-129 — `.tex` chỉ là KHUNG, không phải nguồn số liệu:** chữa 9 dòng lint bắt được là KHÔNG đủ. Bảng xếp hạng 12 quyển, bảng độ đo tổng quát và **5 hình `src/images/chapter4/*.png`** đều dựng từ bộ 120 câu cũ, và repo **không có script sinh lại hình** (`grep -rl matplotlib src/ report/ scripts/` = 0). ▸ **Ba khẳng định SAI SỰ THẬT phải xoá, không phải thay số:** (a) §4.6.1 nói *giáo viên bộ môn đã đánh giá thủ công* + *chú thích do Vintern-1B sinh* — captioner TẮT (D-47), `visual_caption_vi` = 0/938 doc; (b) §4.6 lập luận trên **MiniLM** và coi rerank là *kỳ vọng* trong khi đã đo (D-127); (c) Hạn chế đòi **A100** trong khi máy chạy CPU |
