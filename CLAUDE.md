...
| **Báo cáo (`report/tex_source/`)** | **Chương 1, 2, 3 XONG; 4 xong một nửa** (D-117, D-118, D-119) | Source `.tex` người dùng cung cấp 2026-08-25. ▸ Bìa: tên đề tài lấy **nguyên văn `goal.docx` đã ký** (đọc bằng `python-docx`), loại → **ĐỒ ÁN TỐT NGHIỆP**. ▸ Ch.1 theo MT1–MT5 của đề cương + luận điểm *khoảng cách recall*; Ch.2 thêm mục BM25/RRF/cross-encoder (trước đó BM25 xuất hiện **1 lần** và như phương pháp ĐỐI LẬP); Ch.3 sửa 9 chỗ + viết lại §3.3.2; Ch.4 mục 4.1/4.3/4.4. ▸ **Bảng môi trường cũ ghi SAI**: *Colab Pro + A100 40GB*, thật là máy trạm **CPU** (`torch 2.11.0+cpu`, 16 lõi, 63,9 GB). ▸ **Bỏ dung sai ±1 trang** ở công thức `rel(d)` — lập luận cũ không đúng với pipeline hiện tại. ▸ Máy **không có LaTeX** → `report/kiem_tra_tex.py` kiểm ref/cite/gói/ký tự điều khiển/**số cũ**; hiện **thoát 1 với 19 số cũ còn sót** ở §4.2, §4.5, Ch.5, Tóm tắt (đang chờ số) |
