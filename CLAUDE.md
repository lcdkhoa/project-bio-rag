...
|---|---|---|
| MT1 | Truy vấn đa định dạng: văn bản + **công thức** + hình/sơ đồ/biểu đồ | công thức **BƯỚC 1/3** — gate phát hiện XONG (D-144), Colab MinerU + merge chunk CHƯA (D-56/D-63) |
...
| Test suite | **XANH** | `pytest tests/ -q` → **732 pass, 3 skip** (2026-08-27 sau D-144; các con số **686**, **488**, **584**, **617** ghi ở đây trước đó đã cũ) |
...
| Fingerprint được code sản xuất ĐỌC | **MỘT NỬA** (dòng "CHƯA" cũ là SAI) | Đường MỤC LỤC/manifest **có đọc**: `book/toc.py:418 load_fingerprint`, `toc.py:488` (chọn `toc_pages` + `entry_style`), `toc_lines.py:394`, `manifest.py:77 _fingerprint_offset`. Câu cũ "`grep -rn fingerprint src/` chỉ trúng `image_captioner`" không còn đúng từ D-70. Phần **thật sự chưa dùng**: `min_sat` per-book (`LAYOUT_BOX_MIN_SATURATION` vẫn là hằng số 45) và vùng số trang của `page_number_ocr` |
...
| Xử lý công thức Hoá/Lý (MT1) | **BƯỚC 1/3** (D-144) | gate `is_formula_suspect` XONG, đo được prec 0,8654/rec 1,0000 trên 89 ô/3 NXB. Bước 2 (MinerU qua Colab) + Bước 3 (merge vào chunk) CHƯA làm — cần Colab GPU, D-63 vẫn là bệnh chưa vá |
