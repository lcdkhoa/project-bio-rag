...
| Đánh giá đầu-cuối bộ câu (12 quyển) | **ĐO LẠI XONG trên index mới (D-167, 2026-09-02) — nhưng CHƯA cập nhật vào `report/tex_source/`, và có 3 biến đổi cùng lúc chưa tách được** | `Recall(page)` 0,8782 · `MRR(page)` 0,7850 · `R@10` thô 0,9034 · `Correct` 3,919/5 · `Faithful` 4,202/5 · `Relevancy` 4,443/5 — đo trên **238 câu** (không phải 231 — lệch số có từ trước, D-167 chưa điều tra), index 16 515 chunk `v4_formula_hybrid_fix`, giám khảo Groq (D-163, khác giám khảo cũ OpenRouter). Số cũ (231 câu, D-130, index trước hybrid, giám khảo cũ): Recall 0,9091 · MRR 0,8153 · R@10 0,8961 · Correct 4,065 · Faithful 4,394 · Relevancy 4,602. **KHÔNG được viết "hybrid công thức làm Recall giảm"** — index, giám khảo, VÀ bộ câu hỏi đều đổi cùng lúc, không có lượt đối chứng giữ nguyên hai biến kia. **D-170 (2026-09-02): `ablation.py` THÀNH CÔNG LẦN ĐẦU** (nhờ vá `--build-cache` D-168) — bảng 12 cấu hình + biến thể `n=20`/`fus=norm` đã copy vào `src/test/ablation_report_240.csv`. Bề rộng production (`n=20`, hybrid rerank=on gate=off): MRR 0,8001 · R@1 0,7059 · R@3 0,8824 · R@10 0,9538 — hybrid vẫn thắng bm25/dense thuần ở mọi cột, nhất quán D-82/D-132. `tests/test_bao_cao_so_lieu.py` đang đỏ có chủ đích (số `.tex` chưa khớp CSV mới). **D-168 (phân tích, CHƯA sửa code):** `7_CTST`/`9_CTST` vẫn yếu nhất (0,663/0,677 tổng), **do truy xuất chứ không do trả lời** — nhất quán với D-130 (index/giám khảo cũ), nên là đặc tính cấu trúc của cách CTST được chunk/index (mật độ chunk/trang cao hơn KNTT rõ rệt, ví dụ 7,04 vs 6,03 chunk/trang ở khối 7), không phải nhiễu đo lường. Một ca cụ thể: câu hỏi trang vàng 74 bị retrieval lấy nhầm trang 75 cùng sách, và câu trả lời sai NGƯỢC đáp án chuẩn — gợi ý nội dung/chunk tràn qua ranh giới trang. **Còn nợ: bộ câu hỏi đã đổi số câu hai lần trong ngày** (238→242 D-170 →**240 D-172**, người dùng chốt bỏ 2 khung dư để 12 quyển đều 20 câu/80 câu-NXB) — `evaluator.py`/`recall_at_k.py`/`ablation.py` mới nhất vẫn đo trên 238 câu CŨ trước cả hai lần đổi, cần đo lại toàn bộ. **D-171: đã vá một lỗ hổng trong notebook** trước khi chạy lại — cơ chế khôi phục từ Drive có thể kéo nhầm `_result.csv` ứng với số câu CŨ rồi bị `--bo-qua-da-co` bỏ qua, làm mất câu mới mà không cảnh báo; đã thêm đối chiếu số dòng kết quả vs testset, lệch thì xoá để tính lại (áp dụng đúng cho cả lần đổi số 240 vừa rồi, không cần vá thêm). **D-173 (2026-09-02): lượt chạy 240 câu ĐẦU TIÊN (2 model judge) đã hoàn tất về SỐ DÒNG (đúng 240/240, retrieval đo được: Recall(page)=0,8833 · MRR(page)=0,7889 · R@10 thô=0,9042, khớp hướng D-167) nhưng dính hạn mức TPD giữa chừng — 106/240 câu (44%) trống điểm judge, lệch hẳn về CD/CTST. KHÔNG dùng số judge của lượt này** (Correct/Faithful/Relevancy tính trên 134 câu sống sót sẽ lệch mẫu, không đại diện). Đã mở `EVAL_LLM_MODELS` lên 4 model + vá notebook tự phát hiện quyển thiếu điểm judge và tính lại (xem D-173 trong decision log) — **còn nợ: chạy lại Colab để có bộ 240 câu đủ điểm judge, rồi mới cập nhật `evaluation_report_240.csv`/`report/tex_source/`** |
