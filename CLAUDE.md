...
| Bảng đối chiếu MT4 | **CÓ MỘT NỬA** (D-81, D-82) | Cấu hình 1 của Giai đoạn 3 (**BM25 thuần vs dense thuần vs hybrid** × rerank × cổng lọc) **XONG trên 300 câu / 12 quyển**: `scriptsun_ablation.ps1`, 0 lượt gọi LLM. Ở bề rộng production (20 ứng viên/kênh): **hybrid R@1 0,717 · R@3 0,887 · R@10 0,977 · MRR 0,808** > bm25 0,796 > dense 0,794. **Cấu hình 2 (text-only vs multi-modal) vẫn CHƯA** — `api.py:95` dựng ngữ cảnh chỉ từ `text_docs` nên nó chênh 0 theo cấu trúc. **Hai điều bảng 4 quyển nói SAI và đã bị lật:** (a) “BM25 thuần thắng dense” — trên 300 câu là **hoà** (0,804 vs 0,805); (b) “cổng lọc trung tính” — thật ra **có hại** (hybrid R@10 0,977 → 0,930). Bộ test vẫn **LLM sinh, chưa người duyệt**; phiếu duyệt 50 câu ở `document/review/testset_review_50.csv` |
un_ablation.ps1`, 0 lượt gọi LLM. Đề xuất **hybrid + rerank + rrf** — R@1 0,830 · R@3 0,960 · R@10 **1,000** · MRR 0,898, và là cấu hình DUY NHẤT đạt R@10 = 1,000 **mà không cần rerank**. **Cấu hình 2 (text-only vs multi-modal) vẫn CHƯA** — `api.py:95` dựng ngữ cảnh chỉ từ `text_docs` nên nó chênh 0 theo cấu trúc. **Cảnh báo phải đi kèm mọi số:** bộ test 100 câu / **4/12 quyển** / LLM sinh chưa có người duyệt, và nó **thiên vị kênh từ khoá** (câu hỏi sinh trong lúc đọc chính trang vàng) — nên KHÔNG được viết "BM25 thuần tốt hơn vector" dù bảng cho R@1 0,760 vs 0,600 |
