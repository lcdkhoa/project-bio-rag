...
|---|---|---|
| MT1 | Truy vấn đa định dạng: văn bản + **công thức** + hình/sơ đồ/biểu đồ | **XONG — ETL THẬT ĐÃ CHẠY, ĐO XÁC NHẬN** (D-162). Lượt Colab 8: 4293/16515 chunk merge MinerU thành công (`applied`+`unmatched_count`), 0 lỗi gọi model, đủ 12/12 quyển, BM25 dựng lại khớp. `recall_at_k`/`evaluator.py` đã đo lại trên corpus mới (D-167, 2026-09-02) — còn nợ: bảng `ablation.py` (fail lượt đầu, đã vá, chưa chạy lại) và cập nhật `report/tex_source/` |
...
| Bộ test 240 câu (CBHD kê, 3 bộ × 80) | **238/240 CÂU trong `testsets_240/`; 2 câu bù thêm ĐANG CHỜ người dùng tự duyệt (D-169, 2026-09-02)** | 192 câu văn bản (16/quyển) rút mẫu từ pool 300, 0 lượt LLM, 192/192 gold key khớp index. 48 câu HÌNH dự kiến: 2 khung cắt hỏng vĩnh viễn (`crop hỏng`, `ảnh trang trí không đủ nội dung`) → 46 đã trộn vào `testsets_240/` qua `--ap-dung`. **D-169: đã chọn 2 khung cắt THAY THẾ** (`SGK_KHTN_6_KNTT_p12_01`, `SGK_KHTN_7_CD_p11_01`) bằng `--bu` (bug thật trong `chon()` bắt được + đã vá, xem `document/decision_log.html`), sinh nháp LLM — **chờ người dùng tự mở `document/review/image_questions/phieu.html` duyệt nội dung** rồi mới `--ap-dung` để đạt 240. **CẢNH BÁO còn nguyên cho 46 câu đã có:** toàn bộ câu hỏi/đáp án HÌNH do AI tự mở crop ra xem rồi viết (D-113: không tự động hoá được bằng pixel-derived fields; D-148: AI làm best-effort thay vì để trống) — CHƯA có người thật duyệt độc lập nội dung (chỉ duyệt khung cắt có hỏng không), cần xem lại trước khi trích dẫn trong báo cáo như số liệu chuẩn |
| Đánh giá đầu-cuối bộ câu (12 quyển) | **ĐO LẠI XONG trên index mới (D-167, 2026-09-02) — nhưng CHƯA cập nhật vào `report/tex_source/`, và có 3 biến đổi cùng lúc chưa tách được** | `Recall(page)` 0,8782 · `MRR(page)` 0,7850 · `R@10` thô 0,9034 · `Correct` 3,919/5 · `Faithful` 4,202/5 · `Relevancy` 4,443/5 — đo trên **238 câu** (không phải 231 — lệch số có từ trước, D-167 chưa điều tra), index 16 515 chunk `v4_formula_hybrid_fix`, giám khảo Groq (D-163, khác giám khảo cũ OpenRouter). Số cũ (231 câu, D-130, index trước hybrid, giám khảo cũ): Recall 0,9091 · MRR 0,8153 · R@10 0,8961 · Correct 4,065 · Faithful 4,394 · Relevancy 4,602. **KHÔNG được viết "hybrid công thức làm Recall giảm"** — index, giám khảo, VÀ bộ câu hỏi đều đổi cùng lúc, không có lượt đối chứng giữ nguyên hai biến kia. `ablation.py` (bảng 12 cấu hình BM25/dense/hybrid) **chưa chạy lại được** — lượt Colab đầu fail lặng lẽ (thiếu `--build-cache`, đã vá). `tests/test_bao_cao_so_lieu.py` đang đỏ có chủ đích (số `.tex` chưa khớp CSV mới). **D-168 (phân tích, CHƯA sửa code):** `7_CTST`/`9_CTST` vẫn yếu nhất (0,663/0,677 tổng), **do truy xuất chứ không do trả lời** — nhất quán với D-130 (index/giám khảo cũ), nên là đặc tính cấu trúc của cách CTST được chunk/index (mật độ chunk/trang cao hơn KNTT rõ rệt, ví dụ 7,04 vs 6,03 chunk/trang ở khối 7), không phải nhiễu đo lường. Một ca cụ thể: câu hỏi trang vàng 74 bị retrieval lấy nhầm trang 75 cùng sách, và câu trả lời sai NGƯỢC đáp án chuẩn — gợi ý nội dung/chunk tràn qua ranh giới trang |
