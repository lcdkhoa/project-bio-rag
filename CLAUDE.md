...
- **Config trap on the dev box:** `.env` sets `EMBEDDING_MODEL=./models/paraphrase-multilingual-MiniLM-L12-v2` (384-dim) while the Colab notebook sets `bge-m3` (1024-dim). Indexing with one and querying/appending with the other does not work — pick one (bge-m3, per D-19) and rebuild if you switch.
