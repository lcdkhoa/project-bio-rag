...
- **PARTLY DONE — M3 figures: G4 now measurable and improving, but coverage is 90%, not the 98% target (D-45).** Measured on 16 Bài across all 4 books: **63 figure labels found, 7 provably missing → 90.0% coverage** (was 88.6%), **0 figures assigned to the wrong Bài** (that G4 target is met), largest crop **0.61 → 0.43** of the page with only 1/76 crops above 40%. The gate is `src/test/qa_figures.py` and needs **no hand-labelled pages**: `Hình A.B` means figure B of **Bài A**, so with a contiguous spine you can check both "A equals the page's Bài" and "the B values form 1..max". The missing-count is a **lower bound** (a Bài's last figure going missing drops `max` with it) — never report it as absolute completeness. Three fixed defects: `read_pill` used psm 7 only (`Hình 1.2` and `Hình 1.3` on the same page need psm 8/13 and 7 respectively); `_extract_figure_label` ranked `Em có biết`/`Quan sát` above `Hình N.M` and returned the first match, so box titles stole figure labels; and a caption read twice (pill `Hình 1.9` vs OCR `Hình 19`) survived deduping-by-number, with the broken copy winning the region. Bridging thresholds `_KNTT_BRIDGE_MAX_GAP/_MAX_TEXT` went 0.25 → 0.15 from a 5-config sweep over 24 pages (better on **both** axes, not a trade-off). **Still open:** on a table page OWL-ViT finds each cell correctly but a container box swallows them in `_dedupe_visual_regions`; dropping containers was tried and measured **worse** (4 regions → 2, because `_filter_text_visual_regions` then drops pale glassware on cream) and was reverted. That is the main remaining part of the 90% → 98% gap. Visual QA `src/test/test_image_extraction_full.py` is ported to `PageSource` (it used to render at poppler 150 DPI while the ETL reads native 1094×1536 — different coordinate spaces, so its overlays said nothing about production).
...
- **OPEN — image captioning (Vintern-1B) is silently OFF (D-42).** `ImageCaptioner._load_model` asks for `AutoModelForImageTextToText`, but Vintern-1B is an **InternVL** model registered through `AutoModel` + remote code, so transformers 4.46.3 raises `Unrecognized configuration class InternVLChatConfig`; the `except` swallows it, sets `self.enabled = False` and only logs a warning. Every figure is therefore indexed with **no `caption_vi`** while `IMAGE_CAPTION_ENABLED=true` makes it look enabled — a silent fallback (principle 5). Fixing it also means porting `_generate_caption` to InternVL's `.chat()` API, and measuring Vietnamese caption quality. Do it **after** M3: a correct caption on a wrong crop is still wrong.
- Cost, measured on the dev box (16 cores, 68 GB RAM, **torch 2.11.0+cpu — no CUDA at all**): text OCR ~1.6 s/page → ~21 min for 801 pages; **bge-m3 embedding on CPU 251 ms/chunk** → ~16 min for ~3800 chunks, so a full `--text-only` is **~37–40 min with no GPU**. Image side (OWL-ViT + CLIP on CPU) ~5 s/page → ~70 min. So Colab is optional for ETL; a GPU only really matters for **serving** Qwen2.5-3B.
