...
| Bộ test câu hỏi (CBHD kê 240, mục tiêu gốc) | **242 CÂU — ĐÃ NGƯỜI DÙNG DUYỆT TOÀN BỘ 50 câu hình (D-170, 2026-09-02)** | 192 câu văn bản (16/quyển) rút mẫu từ pool 300, 0 lượt LLM, 192/192 gold key khớp index. 50 câu HÌNH sau duyệt tay thật (41 phút/50 ô, có sửa lỗi nháp LLM — xem D-170): người dùng giữ lại CẢ 2 khung cắt từng bị đánh giá hỏng trước đây (`crop hỏng`, `ảnh trang trí`) thay vì loại, cộng 2 khung cắt thay thế đã bù (D-169, bug `--bu` đã vá) → `SGK_KHTN_6_KNTT`/`SGK_KHTN_7_CD` mỗi quyển 5 hình (10 quyển còn lại giữ 4). `--ap-dung`: `them=50, bo=0`. Tổng **192+50=242**, vượt mục tiêu gốc 240 vì chủ đích không cắt bớt. **CẢNH BÁO còn nguyên:** câu hỏi/đáp án HÌNH do người viết SAU khi có nháp AI (D-113/D-148) — đây LÀ người duyệt độc lập thật (khác 46 câu cũ trước D-170, vốn AI tự best-effort một mình) |
| Đánh giá đầu-cuối bộ câu (12 quyển) | **ĐO LẠI XONG trên index mới (D-167, 2026-09-02) — nhưng CHƯA cập nhật vào `report/tex_source/`, và có 3 biến đổi cùng lúc chưa tách được** | `Recall(page)` 0,8782 · `MRR(page)` 0,7850 · `R@10` thô 0,9034 · `Correct` 3,919/5 · `Faithful` 4,202/5 · `Relevancy` 4,443/5 — đo trên **238 câu** (không phải 231 — lệch số có từ trước, D-167 chưa điều tra), index 16 515 chunk `v4_formula_hybrid_fix`, giám khảo Groq (D-163, khác giám khảo cũ OpenRouter). Số cũ (231 câu, D-130, index trước hybrid, giám khảo cũ): Recall 0,9091 · MRR 0,8153 · R@10 0,8961 · Correct 4,065 · Faithful 4,394 · Relevancy 4,602. **KHÔNG được viết "hybrid công thức làm Recall giảm"** — index, giám khảo, VÀ bộ câu hỏi đều đổi cùng lúc, không có lượt đối chứng giữ nguyên hai biến kia. **D-170 (2026-09-02): `ablation.py` THÀNH CÔNG LẦN ĐẦU** (nhờ vá `--build-cache` D-168) — bảng 12 cấu hình + biến thể `n=20`/`fus=norm` đã copy vào `src/test/ablation_report_240.csv`. Bề rộng production (`n=20`, hybrid rerank=on gate=off): MRR 0,8001 · R@1 0,7059 · R@3 0,8824 · R@10 0,9538 — hybrid vẫn thắng bm25/dense thuần ở mọi cột, nhất quán D-82/D-132. `tests/test_bao_cao_so_lieu.py` đang đỏ có chủ đích (số `.tex` chưa khớp CSV mới). **D-168 (phân tích, CHƯA sửa code):** `7_CTST`/`9_CTST` vẫn yếu nhất (0,663/0,677 tổng), **do truy xuất chứ không do trả lời** — nhất quán với D-130 (index/giám khảo cũ), nên là đặc tính cấu trúc của cách CTST được chunk/index (mật độ chunk/trang cao hơn KNTT rõ rệt, ví dụ 7,04 vs 6,03 chunk/trang ở khối 7), không phải nhiễu đo lường. Một ca cụ thể: câu hỏi trang vàng 74 bị retrieval lấy nhầm trang 75 cùng sách, và câu trả lời sai NGƯỢC đáp án chuẩn — gợi ý nội dung/chunk tràn qua ranh giới trang. **Còn nợ (D-170): bộ câu hỏi vừa lên 242 (từ 238) sau khi người dùng duyệt xong 50 câu hình** — `evaluator.py`/`recall_at_k.py`/`ablation.py` mới nhất vẫn đo trên 238 câu CŨ, ít nhất `SGK_KHTN_6_KNTT`/`SGK_KHTN_7_CD` (hai quyển vừa thêm câu, 20→21) cần đo lại. **242 câu theo NXB: CD 81 · CTST 80 · KNTT 81** (không chia hết 3, phải ghi rõ trong báo cáo). **D-171: đã vá thêm một lỗ hổng trong notebook** trước khi chạy lại — cơ chế khôi phục từ Drive có thể kéo nhầm `_result.csv` ứng với số câu CŨ (đo thấy `6_KNTT` đang lệch 19 vs 21 dòng thật) rồi bị `--bo-qua-da-co` bỏ qua, làm mất câu mới mà không cảnh báo; đã thêm đối chiếu số dòng kết quả vs testset, lệch thì xoá để tính lại |
...
tác dụng khi `EVAL_LLM_MODELS` có ≥2 model.
